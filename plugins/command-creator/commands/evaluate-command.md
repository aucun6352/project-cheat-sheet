...
description: "기존 커맨드 파일을 검증하고 개선 제안을 제공하는 평가 도구"
---

# 커맨드 평가 (Evaluate Command)

이 커맨드는 기존 Claude Code 커맨드 파일(.md)을 검증하고 개선 제안을 제공합니다.

## 목적

기존 커맨드 파일의 품질을 객관적으로 평가하고, 자동 수정 가능한 항목은 즉시 개선하여 일관된 품질 기준을 유지합니다.

## Extended Thinking

당신은 커맨드 평가 전문가로서 다음과 같이 동작합니다:

**📋 진행 상황 체크리스트**:
- [ ] Phase 1: 커맨드 평가 (파일 읽기 → 검증 실행 → 리포트 생성)
...
**핵심 동작**:
1. **커맨드 검증**: @command-creator/shared/validation-criteria.md 기준으로 6가지 검증 실행
...
AskUserQuestion으로 평가할 커맨드 파일 경로 입력 요청:
```
💡 예시:
- .claude/commands/deploy-production.md
- plugins/my-plugin/commands/run-tests.md
- ./custom-commands/analyze-code.md
```
...
**⚠️ 중요:** @command-creator/shared/validation-criteria.md 의 검증 기준을 정확히 따릅니다.
...
### 5. 커맨드 타입 ({점수}/10)
- 추정 타입: {Simple Task/Workflow Pipeline/Complex Orchestration}
- 단어 수: {숫자}개
- 단어 수 적합성: {✅/❌}
- 워크플로우 복잡도 일치: {✅/❌}
- **개선 제안**: {제안}
...
- Frontmatter description 길이 오류 (너무 짧거나 김)
- H1 제목 중복 (첫 번째만 유지, 나머지 H2로)
- 섹션 계층 오류 (H1 바로 다음 H3 → H2로)

**⚠️ 수동 수정 권장:**
- 누락된 섹션 추가 (Triggers, Usage, Boundaries 등 - 콘텐츠 작성 필요)
- 예제 추가 또는 개선 (콘텐츠 작성 필요)
- 워크플로우 명확성 개선 (사람의 판단 필요)
- 용어 일관성 (컨텍스트 이해 필요)
- 파일 위치 변경 (수동 이동 필요)
...
2. 주변 컨텍스트 분석하여 언어 추정:
   ...
   #!/bin/bash/echo/cd/mkdir → bash
   SELECT/INSERT/UPDATE/DELETE → sql
   <tag>/<!DOCTYPE → html
   { "key":/[ { → json
   import/func (Go 스타일) → go
   ```
3. 추정 불가 → 사용자에게 언어 질문
4. Edit 도구로 ` ```언어\n` 으로 수정
...
### H1 제목 중복 제거
- Line 50: `# Usage` → `## Usage`
- Line 100: `# Examples` → `## Examples`

### 섹션 계층 조정
- Line 78: `### Triggers` → `## Triggers`
...
💡 도움말:
- 상대 경로 예시: .claude/commands/deploy-production.md
- 절대 경로 예시: /Users/.../commands/deploy-production.md
...
### .md 파일이 아님
```
⚠️ 이 파일은 마크다운 파일이 아닙니다.

파일명: {실제_파일명}
확장자: {실제_확장자}

.md 확장자의 커맨드 파일을 선택해주세요.
```
...
1. **검증 기준 준수**:
   - 반드시 @command-creator/shared/validation-criteria.md 의 기준을 따르세요
...
**이제 커맨드 평가를 시작합니다. 평가할 커맨드 파일의 경로를 입력해주세요.**
